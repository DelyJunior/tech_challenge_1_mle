...
endpoints = {
    "/add_user": {
        "method": "POST",
        "fields": ["password"],  # username não obrigatório
        "type": "JSON"
    },
    "/api/v1/auth/login": {
        "method": "POST",
        "fields": ["username", "password"],  # username obrigatório
        "type": "Form-data"
    },
...
for field in config["fields"]:
    is_required = field=="username" and rota=="/api/v1/auth/login"
...
if st.button("Enviar Requisição"):
    ...
    try:
        if config["method"] == "GET":
            ...
            if rota=="/api/v1/auth/login" and (not inputs.get("username") or not inputs.get("password")):
